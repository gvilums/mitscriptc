...
int main() {
  ...
  for (int k = 0; k < 100; ++k) {
    root = build(5, heap);
    if (Object::getAlive() != (31 * (k + 1))) {
      std::cerr << "Expected " << (31 * (k + 1)) << " alive objects but got " << Object::getAlive() << std::endl;
      return 1;
    }
    if (!check(root, 5)) {
      return 1;
  ...
  if (Object::getAlive() != 31) {
    std::cerr << "Expected 31 alive object but got " << Object::getAlive() << std::endl;
    return 1;
  }
  if (!check(root, 5)) {
    return 1;
