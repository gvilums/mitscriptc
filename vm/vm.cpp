...
std::pair<ProgVal, ProgVal> get_bin_operands(std::vector<StackVal>& stack) {
    ...
    return std::visit(
        overloaded{[](ProgVal x, ProgVal y) -> std::pair<ProgVal, ProgVal> {
                       return std::pair{x, y};
                   },
                   [](auto x, auto y) -> std::pair<ProgVal, ProgVal> {
                       throw std::string{
                           "stack operands for binary operation must be "
                           "program values"};
                   }},
        lop, rop);
}
...
void VM::exec() {
    ...
    while (this->callstack.size() > 1 ||
           this->callstack.back().iptr <
               this->callstack.back().ctx->instructions.size()) {
        this->step();
    }
}
...
bool VM::step() {
    ...
    if (instr.operation == Operation::LoadConst) {
        ProgVal v =
            value_from_constant(fn->constants_.at(instr.operand0.value()));
        frame->opstack.push_back(v);
        frame->iptr += 1;
    } else if (instr.operation == Operation::LoadGlobal) {
        frame->opstack.push_back(
            globals.at(fn->names_.at(instr.operand0.value())));
        frame->iptr += 1;
        ...
        *ref_cell.ref = val;
        ...
        frame->opstack.push_back(std::visit(
            overloaded{[](int x, int y) -> ProgVal { return x + y; },
                       [](const std::string& l,
                          const std::string& r) -> ProgVal { return l + r; },
                       [](const std::string& l, auto r) -> ProgVal {
                           return l + value_to_string(r);
                       },
                       [](auto l, const std::string& r) -> ProgVal {
                           return value_to_string(l) + r;
                       },
                       [](auto l, auto r) -> ProgVal {
                           throw std::string{"invalid operands for add"};
                       }},
        ...
        if (frame->opstack.size() <= m) {
            throw std::string{
                "error: not enough stack arguments while allocating closure"};
        }
        ...
        *closure =
            Closure{.type = FnType::DEFAULT, .fn = fn, .refs = std::move(refs)};
        ...
        if (frame->opstack.size() <= m) {
            throw std::string{
                "error: not enough stack arguments while calling closure"};
        }
        ...
        if (c.type == FnType::DEFAULT) {
            ...
            this->callstack.push_back(
                StackFrame(c.fn, std::move(args), c.refs));
            ...
            if (!args.empty()) {
                throw std::string{
                    "error: input requires exactly zero arguments"};
            }
            std::string input;
            ...
            if (args.size() != 1) {
                throw std::string{
                    "error: intcast requires exactly one argument"};
            }
            frame->opstack.push_back(
                std::stoi(std::get<std::string>(args.at(0))));
        } else {
        ...
        if (stack_size < 2) {
            throw std::string{
                "error: swap executed on stack with less than 2 elements"};
        } else {
